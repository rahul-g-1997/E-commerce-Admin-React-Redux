...
const AllProducts = () => {
  ...
  function toggleSort() {
    if (isSorted) {
      // If data is already sorted, remove the sort
      dispatch(fetchData(storeDataUnsorted));
      unsorted_toastify();
    } else {
      // If data is not sorted, sort the data
  ...
  // Fetching the data from the API and updating the store
useEffect(() => {
...
}, [dispatch, storeData.length]); // Include dispatch and storeData.length in the dependency array

  return (
    <div className="allProductsContainer">
       <div className="sortDiv">
        <div className="sortButton" onClick={toggleSort}>
          {isSorted ? "UnSort Items" : "Sort Items"}
        </div>
      </div>
      {loading ? (
